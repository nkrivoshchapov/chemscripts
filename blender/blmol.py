# Hosted here: https://github.com/scotthartley/blmol
...
COLORS = {
    ...
    'red': (0.8, 0, 0, 1.0),
    'royal_blue': (0.1, 0.1, 0.85, 1.0),
    'royal_red': (0.9, 0.07, 0.07, 1.0),
...
class Atom:
    ...
    def __init__(self, atomic_number, location, id_num, small=False):
        self.at_num = atomic_number
        self.location = location  # np.array
        self.id_num = id_num
        self.small = small

    def draw(self, color='by_element', radius=None, units='A',
...
class Bond:
    ...
    def _draw_half(location, length, rot_angle, rot_axis, element,
                   radius=0.2, color='by_element', units='A',
    ...
    def draw(self, radius=0.2, color='by_element', units='A',
             vertices=64, edge_split=False):
        ...
        # print("%d & %d -- %s & %s" % (self.atom1.id_num, self.atom2.id_num, repr(self.atom1.small), repr(self.atom2.small)))
        if self.atom1.small or self.atom2.small:
...
class Molecule:
    ...
    def __init__(self, name='molecule', atoms=None, bonds=None, small_atoms=None):
        self.name = name
        if atoms is None:
            self.atoms = []
        else:
    ...
    def set_alignment(self, rot, trans):
        for atom in self.atoms:
            atom.location = rot @ atom.location + trans
    ...
    def draw_bonds(self, caps=True, radius=0.2, color='by_element',
                   units='A', join=True, with_H=True, subsurf_level=1,
                   vertices=64, edge_split=False):
    ...
    def draw_atoms(self, color='by_element', radius=None, units='A',
    ...
    def read_sdf(self, filename):
        sdflines = open(filename, "r").readlines()
        natoms = int(sdflines[3][0:3])
        nbonds = int(sdflines[3][3:6])
        for i in range(4, 4 + natoms):
            parts = sdflines[i].replace("\n", "").split()
            coords = np.array([float(parts[0]), float(parts[1]), float(parts[2])])
            idnum = i - 3
            atnum = ATOMIC_NUMBERS[parts[3].strip().upper()]
            self.add_atom(Atom(atnum, coords, idnum, small=idnum in self.small_atoms))
